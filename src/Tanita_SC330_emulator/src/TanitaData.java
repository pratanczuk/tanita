...
public class TanitaData {
	...
	{
		// validate header
		return aData.startsWith( getControlString().toString());
	}
   ...
   {
	   List<OneEntry> datalist = new ArrayList<OneEntry>();
	   ...
	   for (OneEntry item : datalist) {
		   System.out.println( item.Name + "," + item.ID + "," + item.Value + "," + item.Unit + ","+item.JavaScriptID);
	   }
   ...
   {
	   DataDefinitionList = new ArrayList<OneEntry>();
		   String stringRead;
       
		   for( int i = 0; i < DataDefinition.length; i++)
		   {
			   stringRead = DataDefinition[i];
			   String[] elements = stringRead.split(";");
   		...
   		private String DataDefinition[] = 
   			{
   				...
   				"BMR (kJ);rb;XXXXX;kJ;edit­field­fit­body­energy­needs­und­0­value",
   				"BMR (kcal);rB;XXXXX;kcal; ",
   				"BMR score;rJ;XX; ; ",
